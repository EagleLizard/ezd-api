...
async function getProject(projectRoute: string) {
  let queryStr = `
    SELECT
      jp.project_key, jp.route, jp.title, jp.project_date,
      v."name" as venue,
    ...
    FROM jcd_project jp
      ...
      INNER JOIN jcd_project_description jpd
        ON jp.jcd_project_id = jpd.jcd_project_id
